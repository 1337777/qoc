...
(**TODO: get or set the value of original Printmod option "Short" "Module" "Printing" instead ? **)
let () =
  declare_bool_option
    { optdepr  = false;
...
let print_body is_impl extent env mp (l,body) =
  ...
  hov 2 (match body with
	...
	| Def _ -> def "定义" (* "定义" "dingyi" ; OLD: "Definition" *) ++ spc ()
	| OpaqueDef _ when is_impl -> def "Theorem" ++ spc ()
	| _ -> def "参数" (* "参数" "canshu" ; OLD: "Parameter" *) ++ spc ()) ++ name ++
      (match extent with
         | OnlyNames -> mt ()
         | WithContents ->
    ...
    | SFBmind mib ->
      ...
      | OnlyNames ->
        let keyword =
          let open Declarations in
          match mib.mind_finite with
          | Finite -> def "归纳义" (* "归纳义" "guinayi" ; "归纳以" "guinayi" ; OLD: "Inductive" *)
...
let print_structure is_type extent env mp locals struc =
  ...
  let kwd = if is_type then "结构签" (* "结构签" "jiegou qian" ; OLD: "Sig" *) else "结构体" (* "结构体" "jiegou ti" ; OLD: "Struct" *) in
  hv 2 (keyword kwd ++ spc () ++ print_struct false extent env' mp struc ++
	brk (1,-2) ++ keyword "结束" (* "结束" "jieshu" ; OLD: "End" *) )
...
let rec print_typ_expr extent env mp locals mty =
  ...
  | MEwith(me,WithDef(idl,(c, _)))->
      ...
      hov 2 (print_typ_expr extent env mp locals me ++ spc() ++ str "with" ++ spc()
             ++ def "定义" (* "定义" "dingyi" ; OLD: "Definition" *) ++ spc() ++ str s ++ body)
